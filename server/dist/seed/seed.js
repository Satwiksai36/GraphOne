...
async function main() {
    ...
    const companiesList = [...baseCompanies];
    ...
    for (const comp of finalCompanies) {
        const { ownership, openJobs, ...dbComp } = comp;
    ...
    // OpenAI Competitors
    // openai -> anthropic (competitorId: anthropic)
    await prisma.competitor.create({ data: { companyId: 'openai', competitorId: 'anthropic', isAdjacent: false } });
    await prisma.competitor.create({ data: { companyId: 'openai', competitorId: 'google-deepmind', isAdjacent: false } });
    await prisma.competitor.create({ data: { companyId: 'openai', competitorId: 'perplexity', isAdjacent: true } });
    await prisma.competitor.create({ data: { companyId: 'openai', competitorId: 'cursor', isAdjacent: true } });
